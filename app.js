...
`+ OPTS(id, S('cmp_lvl', 'Compression Level', [['low', 'Low — Lossless structure only (no quality loss)'], ['med', 'Medium — Colour recompress at 50% quality'], ['hi', 'High — 25% quality (most aggressive)']], 'med')) + `<div style="padding:0.6rem 0.875rem;background:rgba(139,92,246,0.06);border:1px solid rgba(139,92,246,0.18);border-radius:10px;font-size:0.78rem;color:var(--muted);margin-bottom:0.5rem">💡 Always picks the best result — if rasterising makes a file bigger, the lossless version is used instead.</div>`);
    ...
    case 'pdf2ppt': return base(`<div style="padding:0.9rem 1rem;background:rgba(249,115,22,0.06);border:1px solid rgba(249,115,22,0.18);border-radius:10px;font-size:0.82rem;color:#c2410c;margin-bottom:1rem">🎞️ Builds a browser-generated PPTX with one slide per PDF page, extracted titles, bullet text, and editable text boxes.</div>` + OPTS(id, S('p2p_ratio', 'Slide Size', [['wide', 'Widescreen 16:9'], ['standard', 'Standard 4:3']], 'wide') + S('p2p_title', 'Title Source', [['firstline', 'Use first line as slide title'], ['pagename', 'Use page number as title']], 'firstline') + S('p2p_body', 'Body Layout', [['bullets', 'Bullet points'], ['paragraphs', 'Paragraph blocks']], 'bullets') + S('p2p_density', 'Text Density', [['brief', 'Brief summary'], ['balanced', 'Balanced'], ['full', 'Full extraction']], 'balanced')));
...
function label(id) {
  const m = {
    merge: '🔗 Merge PDFs', split: '✂️ Split PDF', removepg: '🗑️ Delete Pages', extract: '📤 Extract Pages', organize: '📋 Apply Order', compress: '🗜️ Compress PDF', repair: '🔧 Repair PDF', ocr: '🔡 Extract Text', jpg2pdf: '📷 Convert to PDF', word2pdf: '📝 Convert to PDF', ppt2pdf: '📊 Convert to PDF', xls2pdf: '📈 Convert to PDF', html2pdf: '🌐 Convert to PDF', pdf2jpg: '🖼️ Convert to Images', pdf2ppt: '🎞️ Convert to PowerPoint', pdf2word: '📄 Convert to Word', pdf2xls: '📊 Export to CSV', pdf2pdfa: '🗂️ Convert to PDF/A', rotate: '🔄 Rotate Pages', watermark: '💧 Apply Watermark', pagenums: '🔢 Add Page Numbers', crop: '🔲 Crop Pages', editpdf: '✏️ Save Annotated PDF', unlock: '🔓 Unlock PDF', protect: '🔐 Protect PDF', sign: '✍️ Add Signature', redact: '⬛ Apply Redactions', compare: '🔀 Compare PDFs',
...
async function run(id) {
  ...
  try {
    switch (id) {
      ...
      case 'pdf2ppt': await doPdf2Ppt(s); break;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  topAd.innerHTML = `
    <ins class="adsbygoogle"
         style="display:block"
         data-ad-client="ca-pub-4413784450447798"
